...
for idx, row in top_combinations.iterrows():
    ...
    simulated_revenue = np.clip(simulated_revenue, 0, None)
    sns.kdeplot(simulated_revenue, label=f"{row['neighbourhood_group_cleansed']} – {row['room_type']}")
...
# from sklearn.pipeline import Pipeline
# 
# st.title("📊 Airbnb Price and Revenue Estimator")
# 
# # user inputs
...
# 
# 
# st.subheader("💰 Pricing & Revenue Forecast")
# st.write(f"**Recommended Nightly Price**: ${predicted_price:.2f}")
# st.write(f"**Simulated Annual Revenue**: ${simulated_revenue.mean():.2f}")
# st.write(f"**Revenue Range (±1 SD)**: ${simulated_revenue.mean() - simulated_revenue.std():.2f} – ${simulated_revenue.mean() + simulated_revenue.std():.2f}")
#
